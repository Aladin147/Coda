...
class PersonalityParameterManager(PersonalityParameterInterface):
    ...
    def __init__(self, config: Optional[PersonalityParameterConfig] = None):
        ...
        logger.info(
            f"PersonalityParameterManager initialized with {len(self.parameters.traits)} traits"
        )
    ...
    def adjust_trait(
        ...
        logger.info(
            f"Adjusted {trait_type.value}: {old_value:.3f} → {new_value:.3f} (Δ{actual_delta:+.3f}) - {reason}"
        )
    ...
    def validate_trait_values(self) -> List[str]:
        ...
        for trait_type, trait in self.parameters.traits.items():
            if not (trait.min_value <= trait.value <= trait.max_value):
                issues.append(
                    f"{trait_type.value} value {trait.value} is outside range [{trait.min_value}, {trait.max_value}]"
                )
    ...
    def export_parameters(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "adjustment_history": [
                adj.model_dump() for adj in self.adjustment_history[-50:]
            ],  # Last 50 adjustments
