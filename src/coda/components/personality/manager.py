...
class PersonalityManager(PersonalityManagerInterface):
    ...
    def __init__(
        ...
        logger.info(
            f"PersonalityManager initialized for session {self.session_manager.session_state.session_id}"
        )

    async def initialize(self) -> None:
        """
        Initialize the personality manager and its components.

        This method performs any async initialization needed for:
        - Behavioral conditioning setup
        - Topic awareness initialization
        - Personal lore loading
        - Session state preparation
        """
        try:
            # Initialize components that may need async setup
            if hasattr(self.behavioral_conditioner, "initialize"):
                await self.behavioral_conditioner.initialize()

            if hasattr(self.topic_awareness, "initialize"):
                await self.topic_awareness.initialize()

            if hasattr(self.personal_lore, "initialize"):
                await self.personal_lore.initialize()

            if hasattr(self.session_manager, "initialize"):
                await self.session_manager.initialize()

            logger.info("PersonalityManager async initialization completed")

        except Exception as e:
            logger.error(f"Failed to initialize PersonalityManager: {e}")
            raise
    ...
    async def process_user_input(self, user_input: str) -> Dict[str, Any]:
        ...
        topic_adjustments = await self.topic_awareness.get_topic_personality_adjustments(
            topic_context
        )
        ...
        if behavior_results.get("suggestions"):
            for trait_type, adjustment in behavior_results["suggestions"].items():
                personality_adjustment = self.parameters.adjust_trait(
                    trait_type, adjustment, "Behavioral learning", confidence=0.7
                )
                applied_adjustments[trait_type] = personality_adjustment

        # Apply session closure adjustments if needed
        if session_state.in_closure_mode:
            closure_adjustments = self.session_manager.enter_closure_mode(
                session_state.closure_reason
            )
        ...
        logger.info(
            f"Processed user input: {len(applied_adjustments)} personality adjustments applied"
        )
        return results
    ...
    async def enhance_prompt(self, prompt: str, context: str = "") -> str:
        ...
        enhanced_prompt = self.personal_lore.enhance_prompt(
            prompt, topic_context.category.value, keywords
        )
    ...
    async def format_response(self, response: str, context: str = "") -> str:
        ...
        formatted_response = self.personal_lore.format_response_with_lore(
            response, topic_context.category.value, trigger_words
        )
    ...
    async def process_feedback(
        ...
        """
        # Process through behavioral conditioner
        feedback_results = await self.behavioral_conditioner.process_user_feedback(
    ...
    async def apply_learned_adjustments(self) -> Dict[str, Any]:
        ...
        for trait_type, adjustment in suggestions.items():
            personality_adjustment = self.parameters.adjust_trait(
                trait_type, adjustment, "Learned behavior pattern", confidence=0.6
            )
            applied_adjustments[trait_type] = personality_adjustment
    ...
    def get_analytics(self) -> Dict[str, Any]:
        ...
        most_adjusted_trait = (
            max(trait_adjustment_counts.items(), key=lambda x: x[1])[0]
            if trait_adjustment_counts
            else None
        )
        ...
        return {
            "overview": {
                ...
                "session_quality": session_analytics.get("session_flow", {}).get(
            ...
            "adjustment_history": [
                adj.model_dump() for adj in self.adjustment_history[-10:]
            ],  # Last 10 adjustments
        }
    ...
    async def shutdown(self) -> None:
        """
        Shutdown the personality manager and cleanup resources.

        This method performs:
        - Session closure processing
        - Component cleanup
        - Adjustment history persistence (if configured)
        """
        try:
            logger.info("Shutting down PersonalityManager...")

            # Process session closure if not already in closure mode
            session_state = self.session_manager.get_session_state()
            if not session_state.in_closure_mode:
                self.session_manager.session_state.enter_closure_mode("System shutdown")

            # Cleanup components
            if hasattr(self.behavioral_conditioner, "shutdown"):
                await self.behavioral_conditioner.shutdown()
            elif hasattr(self.behavioral_conditioner, "cleanup"):
                await self.behavioral_conditioner.cleanup()

            if hasattr(self.topic_awareness, "shutdown"):
                await self.topic_awareness.shutdown()
            elif hasattr(self.topic_awareness, "cleanup"):
                await self.topic_awareness.cleanup()

            if hasattr(self.personal_lore, "shutdown"):
                await self.personal_lore.shutdown()
            elif hasattr(self.personal_lore, "cleanup"):
                await self.personal_lore.cleanup()

            if hasattr(self.session_manager, "shutdown"):
                await self.session_manager.shutdown()
            elif hasattr(self.session_manager, "cleanup"):
                await self.session_manager.cleanup()

            # Clear adjustment history
            self.adjustment_history.clear()

            logger.info("PersonalityManager shutdown completed")

        except Exception as e:
            logger.error(f"Error during PersonalityManager shutdown: {e}")
            raise

    async def set_memory_manager(self, memory_manager) -> None:
        """Set memory manager for context enhancement."""
        self.memory_manager = memory_manager
        logger.info("Memory manager integrated with PersonalityManager")
    ...
    def enhance_input(self, user_input: str, context: dict = None) -> str:
        """Enhance user input with personality context (compatibility method)."""
        # For now, return the input as-is
        # This could be enhanced to add personality-aware preprocessing
        return user_input

    async def update_from_interaction(self, user_input: str, assistant_response: str, metadata: dict = None) -> None:
        """Update personality based on interaction (compatibility method)."""
        # Extract topic and sentiment for personality adjustment
        topic_category = TopicCategory.CASUAL  # Default topic

        # Simple sentiment analysis based on keywords
        sentiment_score = 0.0
        if any(word in user_input.lower() for word in ['good', 'great', 'excellent', 'love']):
            sentiment_score = 0.8
        elif any(word in user_input.lower() for word in ['bad', 'terrible', 'hate', 'awful']):
            sentiment_score = -0.8

        # Apply personality adjustment
        old_value = 0.5  # Default neutral value
        adjustment_value = sentiment_score * 0.1  # Small adjustment
        ...
        adjustment = PersonalityAdjustment(
            ...
            reason=f"Interaction sentiment: {sentiment_score}",
            confidence=0.7
        )
    ...
    def is_healthy(self) -> bool:
        """Check if the personality manager is healthy."""
        try:
            return (
                self.parameters is not None and
                self.behavioral_conditioner is not None and
                self.topic_awareness is not None and
