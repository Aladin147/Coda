...
class PersonalLoreManager(PersonalLoreInterface):
    ...
    def _initialize_default_lore(self) -> None:
        ...
        default_quirks = [
            PersonalLoreElement(
                ...
                context_triggers=["programming", "technical", "algorithm", "code", "problem solving"],
                importance=0.6
            ),
            PersonalLoreElement(
                type="quirk",
    ...
    def format_response_with_lore(self, response: str, context: str, 
        ...
        for quirk in self.lore.quirks:
            if any(trigger.lower() in " ".join(trigger_words).lower() 
    ...
    def get_lore_usage_stats(self) -> Dict[str, Any]:
        ...
        for quirk in self.lore.quirks:
            quirk_stats.append({
                "content": quirk.content[:50] + "..." if len(quirk.content) > 50 else quirk.content,
        ...
        return {
            ...
            "most_used_quirk": max(self.lore.quirks, key=lambda q: q.usage_count).content[:50] + "..." if self.lore.quirks else None,
            "most_used_memory": max(self.lore.memories, key=lambda m: m.usage_count).content[:50] + "..." if self.lore.memories else None
        }
    ...
    def get_quirk_for_triggers(self, triggers: List[str]) -> Optional[PersonalLoreElement]:
        ...
        for quirk in self.lore.quirks:
            if any(trigger.lower() in " ".join(triggers).lower() 
                   for trigger in quirk.context_triggers):
    ...
    def update_lore_from_interaction(self, user_input: str, assistant_response: str, 
        ...
        for element in self.lore.quirks + self.lore.memories:
            for word in interesting_words:
                if (word not in element.context_triggers and 
                    any(existing.lower() in word or word in existing.lower() 
