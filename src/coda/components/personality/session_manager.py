...
class SessionManager(SessionManagerInterface):
    ...
    def generate_session_summary(self) -> Dict[str, Any]:
        ...
        avg_engagement = sum(self.user_engagement_scores) / len(self.user_engagement_scores) if self.user_engagement_scores else 0.0
        ...
        avg_response_time = sum(interaction_intervals) / len(interaction_intervals) if interaction_intervals else 0.0
    ...
    def get_session_analytics(self) -> Dict[str, Any]:
        ...
        user_message_lengths = [len(i["content"]) for i in self.interaction_history if i["role"] == "user"]
        assistant_message_lengths = [len(i["content"]) for i in self.interaction_history if i["role"] == "assistant"]
        
        # Timing patterns
        interaction_times = [i["timestamp"] for i in self.interaction_history]
        if len(interaction_times) > 1:
            intervals = [(interaction_times[i] - interaction_times[i-1]).total_seconds() 
        ...
        return {
            ...
            "engagement_analysis": {
                "average_engagement": sum(self.user_engagement_scores) / len(self.user_engagement_scores) if self.user_engagement_scores else 0,
    ...
    def _analyze_user_engagement(self, content: str) -> float:
        ...
        # Positive words
        positive_words = ['great', 'awesome', 'excellent', 'love', 'amazing', 'fantastic', 'wonderful']
    ...
    def _detect_natural_ending(self) -> bool:
        ...
        recent_user_messages = [i["content"] for i in self.interaction_history[-4:] if i["role"] == "user"]
        
        ending_phrases = [
            'thanks', 'thank you', 'goodbye', 'bye', 'see you', 'that\'s all',
            'that helps', 'perfect', 'got it', 'understood', 'that\'s enough'
        ]
    ...
    def _determine_session_phase(self) -> str:
        ...
        if self.session_state.turn_count <= 2:
            ...
            return "closing"
        elif self.session_state.duration_seconds > self.long_session_threshold.total_seconds() * 0.7:
            return "late"
        else:
            return "active"
