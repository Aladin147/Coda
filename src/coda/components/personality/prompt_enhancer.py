...
"""
    ...
    def enhance_system_prompt(self, 
                            base_prompt: str,
        ...
        if enhancement_level == "minimal":
            ...
            return self._enhance_moderate(base_prompt, personality_params, topic_context, session_state)
        else:  # full
            return self._enhance_full(base_prompt, personality_params, personal_lore, topic_context, session_state)
    
    def enhance_user_prompt(self,
                           user_prompt: str,
                           personality_params: PersonalityParameters,
                           topic_context: TopicContext,
                           relevant_lore: List[Any] = None) -> str:
        """
        ...
        if len(user_prompt) > 200:  # Complex request
            style_guidance = self._generate_response_style_guidance(personality_params, topic_context)
            if style_guidance:
    ...
    def _enhance_minimal(self, base_prompt: str, personality_params: PersonalityParameters) -> str:
        ...
        if significant_traits:
            personality_note = f"Your current personality leans toward being {', '.join(significant_traits[:3])}."
            return f"{base_prompt}\n\n{personality_note}"
        
        return base_prompt
    
    def _enhance_moderate(self, 
                         base_prompt: str,
                         personality_params: PersonalityParameters,
                         topic_context: TopicContext,
        ...
        if topic_context.current_topic:
            context_info.append(f"Current topic: {topic_context.current_topic} ({topic_context.category.value})")
    ...
    def _enhance_full(self,
        ...
        style_guidelines = self._generate_detailed_style_guidelines(personality_params, topic_context)
        if style_guidelines:
    ...
    def _generate_detailed_trait_descriptions(self, personality_params: PersonalityParameters) -> str:
    ...
    def _generate_lore_section(self, personal_lore: PersonalLore, topic_context: TopicContext) -> str:
        ...
        # Relevant quirks
        relevant_quirks = personal_lore.get_relevant_lore(topic_context.category.value, topic_context.keywords)
    ...
    def _generate_session_info(self, session_state: SessionState) -> str:
        ...
        info_parts = [
            f"{session_state.turn_count} turns",
    ...
    def _generate_response_style_guidance(self, personality_params: PersonalityParameters, topic_context: TopicContext) -> str:
    ...
    def _generate_detailed_style_guidelines(self, personality_params: PersonalityParameters, topic_context: TopicContext) -> str:
