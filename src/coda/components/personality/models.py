...
class PersonalityTrait(BaseModel):
    ...
    def validate_value_range(cls, v, values):
        ...
        if not (min_val <= v <= max_val):
            raise ValueError(f"Value {v} must be between {min_val} and {max_val}")
        return v
...
class PersonalityParameters(BaseModel):
    ...
    def adjust_trait(self, trait_type: PersonalityTraitType, delta: float, reason: str = "") -> bool:
...
class TopicContext(BaseModel):
    ...
    def update_topic(self, topic: str, category: TopicCategory, confidence: float, keywords: List[str]):
...
class PersonalityConfig(BaseModel):
    ...
    behavioral_conditioning: BehavioralConditioningConfig = Field(default_factory=BehavioralConditioningConfig)
