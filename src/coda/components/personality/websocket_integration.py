...
class WebSocketPersonalityManager(PersonalityManager, WebSocketPersonalityInterface):
    ...
    async def broadcast_adjustment(self, adjustment: PersonalityAdjustment) -> None:
        ...
        """
        await self.broadcast_personality_event("trait_adjustment", {
    ...
    async def process_user_input(self, user_input: str) -> Dict[str, Any]:
        ...
        if topic_context.get("current_topic"):
            await self.broadcast_personality_event("topic_detected", {
        ...
        if session_state.get("in_closure_mode"):
            await self.broadcast_personality_event("session_closure", {
    ...
    async def process_assistant_response(self, response: str) -> Dict[str, Any]:
        ...
        if behavior_patterns:
            await self.broadcast_personality_event("behavior_patterns_detected", {
    ...
    async def process_feedback(self, feedback: str, feedback_type: str = "general") -> Dict[str, Any]:
        ...
        await self.broadcast_personality_event("feedback_processed", {
        ...
        adjustments = results.get("personality_adjustments", {})
        for trait_name, adjustment_data in adjustments.items():
            await self.broadcast_personality_event("feedback_adjustment", {
                "trait": trait_name,
                "adjustment": adjustment_data,
                "feedback_type": feedback_type
            })
    ...
    async def apply_learned_adjustments(self) -> Dict[str, Any]:
        ...
        await self.broadcast_personality_event("learning_applied", {
    ...
    async def broadcast_trait_evolution(self, trait_name: str, days: int = 7) -> None:
        ...
        trait_adjustments = [
            ...
            if adj.trait_type.value == trait_name
        ]
        
        # Create evolution data
        evolution_data = []
        current_value = self.parameters.get_trait_value(getattr(self.parameters.PersonalityTraitType, trait_name.upper()))
        ...
        await self.broadcast_personality_event("trait_evolution", {
            "trait": trait_name,
    ...
    async def broadcast_behavior_insights(self) -> None:
        ...
        insights = {
            "learned_preferences": {k.value: v for k, v in behavior_profile.user_preferences.items()},
            "confidence_scores": {k.value: v for k, v in behavior_profile.confidence_scores.items()},
            "learning_stats": learning_stats,
            "detected_patterns": [p.model_dump() for p in behavior_profile.detected_patterns[-5:]]  # Last 5 patterns
        }
    ...
    def reset_session(self, new_session_id: str = None) -> None:
        ...
        if self.websocket_integration:
            import asyncio
            asyncio.create_task(self.broadcast_personality_event("session_reset", {
