...
class WebSocketLLMManager(LLMManager, WebSocketLLMInterface):
    ...
    async def broadcast_response_start(
        ...
        Args:
            ...
            model: Model name
        """
        await self.broadcast_llm_event("response_start", {
    ...
    async def broadcast_response_chunk(
        ...
        Args:
            ...
            chunk: Stream chunk
        """
        await self.broadcast_llm_event("response_chunk", {
            "conversation_id": conversation_id,
    ...
    async def broadcast_response_complete(
        ...
        """
        await self.broadcast_llm_event("response_complete", {
            "conversation_id": conversation_id,
    ...
    async def broadcast_function_calls(
        ...
        """
        await self.broadcast_llm_event("function_calls", {
            "conversation_id": conversation_id,
            "function_calls": [
                {
    ...
    async def generate_response(
        ...
        **kwargs
        ...
        await self.broadcast_llm_event("request_start", {
            "conversation_id": conversation_id,
        ...
        try:
            ...
            if stream:
                ...
                await self.broadcast_response_complete(
                    response.conversation_id or conversation_id or "unknown",
                    response
                )
            ...
            # Broadcast error
            await self.broadcast_llm_event("request_error", {
                "conversation_id": conversation_id,
                "error": str(e),
                "timestamp": datetime.now().isoformat()
            })
        ...
        **kwargs
        ...
        try:
            async for chunk in super().generate_response(
                ...
                await self.broadcast_response_chunk(
                    conversation_id or "unknown",
                    chunk
                )
            ...
            await self.broadcast_llm_event("stream_error", {
                "conversation_id": conversation_id,
                "error": str(e),
                "timestamp": datetime.now().isoformat()
            })
    ...
    async def broadcast_conversation_created(self, conversation: LLMConversation) -> None:
        ...
        await self.broadcast_llm_event("conversation_created", {
    ...
    async def broadcast_conversation_updated(self, conversation: LLMConversation) -> None:
        ...
        await self.broadcast_llm_event("conversation_updated", {
            "conversation_id": conversation.conversation_id,
    ...
    async def broadcast_provider_status_change(self, provider: str, status: Dict[str, Any]) -> None:
        ...
        await self.broadcast_llm_event("provider_status_change", {
            "provider": provider,
            "status": status,
            "timestamp": datetime.now().isoformat()
        })
    ...
    async def broadcast_analytics_update(self) -> None:
        ...
        await self.broadcast_llm_event("analytics_update", {
            "analytics": analytics,
            "timestamp": datetime.now().isoformat()
        })
    ...
    async def _broadcast_initial_state(self) -> None:
        ...
        await self.broadcast_llm_event("system_config", {
    ...
    async def cleanup(self) -> None:
        ...
        if self._websocket_integration:
            await self.broadcast_llm_event("llm_system_shutdown", {
                "timestamp": datetime.now().isoformat()
            })
