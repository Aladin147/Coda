...
class BaseLLMProvider(LLMProviderInterface):
    ...
    def __init__(self, config: ProviderConfig):
        ...
        logger.info(f"Initialized {self.get_provider_name().value} provider with model: {config.model}")
    ...
    async def generate_response(
        ...
        **kwargs
        ...
        if not self.is_available():
            raise LLMError(f"Provider {self.get_provider_name().value} is not available", self.get_provider_name().value)
        ...
        try:
            ...
            if stream:
                return await self._generate_streaming_with_retries(messages, functions, **params)
            else:
            ...
            logger.error(f"Provider {self.get_provider_name().value} generation failed: {e}")
    ...
    async def validate_connection(self) -> bool:
        ...
        try:
            ...
            await asyncio.wait_for(
                self._make_request(test_messages),
                timeout=10.0
            )
            ...
            logger.warning(f"Connection validation failed for {self.get_provider_name().value}: {e}")
        ...
        **kwargs
        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                ...
                logger.warning(f"Provider {self.get_provider_name().value} timeout (attempt {attempt + 1}/{self.config.max_retries + 1})")
                ...
                last_error = str(e)
                logger.warning(f"Rate limit hit for {self.get_provider_name().value} (attempt {attempt + 1}/{self.config.max_retries + 1})")
                ...
                logger.warning(f"Provider {self.get_provider_name().value} error: {e} (attempt {attempt + 1}/{self.config.max_retries + 1})")
        ...
        raise LLMError(last_error or "Request failed after all retries", self.get_provider_name().value)
