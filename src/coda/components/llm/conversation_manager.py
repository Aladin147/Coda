...
class ConversationManager(ConversationManagerInterface):
    ...
    async def add_message(self, conversation_id: str, message: LLMMessage) -> None:
        ...
        if (len(conversation.messages) >= self.config.summarize_threshold and 
    ...
    async def get_conversation_context(
        ...
        logger.debug(f"Retrieved context for {conversation_id}: {len(context_messages)} messages, ~{current_tokens} tokens")
