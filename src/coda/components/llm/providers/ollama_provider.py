    ...
    async def _make_request(
        ...
        **kwargs,
        ...
        try:
            ...
            # Configure SSL context for local development
            ssl_context = None
            if self.base_url.startswith("http://"):
                # For HTTP connections, no SSL needed
                ssl_context = False
            elif self.base_url.startswith("https://localhost") or "127.0.0.1" in self.base_url:
                # For local HTTPS, create permissive SSL context
            ...
            async with aiohttp.ClientSession(connector=connector) as session:
                ...
                async with session.post(
                    ...
                    if response.status != 200:
                        ...
                        raise LLMError(
                            f"Ollama API error: {response.status} - {error_text}", "ollama"
                        )
            ...
            # Check if it's a timeout error (aiohttp.ClientTimeout has inheritance issues)
            if "timeout" in str(e).lower() or "ClientTimeout" in str(type(e).__name__):
                raise LLMTimeoutError(
                    f"Ollama request timed out after {self.config.timeout} seconds", "ollama"
                )
            else:
