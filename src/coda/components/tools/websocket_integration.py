...
from .models import ToolDefinition, ToolCall, ToolResult, ToolExecution, ToolEvent, ToolConfig
...
class WebSocketToolManager(ToolManager, WebSocketToolInterface):
    ...
    async def broadcast_tool_execution(self, execution: ToolExecution, result: ToolResult) -> None:
        ...
        """
        await self.broadcast_tool_event("tool_execution_completed", {
    ...
    async def broadcast_tool_registration(self, tool_definition: ToolDefinition) -> None:
        ...
        """
        await self.broadcast_tool_event("tool_registered", {
    ...
    async def process_function_call(self, function_call: Dict[str, Any]) -> ToolResult:
        ...
        await self.broadcast_tool_event("function_call_received", {
        ...
        await self.broadcast_tool_event("function_call_completed", {
    ...
    async def broadcast_tool_analytics(self) -> None:
        ...
        await self.broadcast_tool_event("tool_analytics_update", {
            "analytics": analytics,
            "timestamp": datetime.now().isoformat()
        })
    ...
    async def broadcast_performance_metrics(self) -> None:
        ...
        await self.broadcast_tool_event("tool_performance_metrics", {
            "metrics": metrics,
            "timestamp": datetime.now().isoformat()
        })
    ...
    async def broadcast_registry_stats(self) -> None:
        ...
        await self.broadcast_tool_event("tool_registry_stats", {
            "stats": stats,
            "timestamp": datetime.now().isoformat()
        })
    ...
    async def broadcast_execution_stats(self) -> None:
        ...
        await self.broadcast_tool_event("tool_execution_stats", {
            "stats": stats.model_dump(),
            "timestamp": datetime.now().isoformat()
        })
    
    async def broadcast_tool_error(self, tool_name: str, error: str, call_id: Optional[str] = None) -> None:
        """
        ...
        """
        await self.broadcast_tool_event("tool_error", {
            "tool_name": tool_name,
            "error": error,
            "call_id": call_id,
            "timestamp": datetime.now().isoformat()
    ...
    async def broadcast_tool_unregistered(self, tool_name: str) -> None:
        ...
        """
        await self.broadcast_tool_event("tool_unregistered", {
            "tool_name": tool_name,
            "timestamp": datetime.now().isoformat()
        })
    ...
    async def broadcast_tool_availability_change(self, tool_name: str, available: bool) -> None:
        ...
        """
        await self.broadcast_tool_event("tool_availability_changed", {
            "tool_name": tool_name,
            "available": available,
            "timestamp": datetime.now().isoformat()
        })
    ...
    async def _broadcast_initial_state(self) -> None:
        ...
        await self.broadcast_tool_event("tools_available", {
            "tools": [
                {
                    "name": tool.name,
    ...
    async def cleanup(self) -> None:
        ...
        if self._websocket_integration:
            await self.broadcast_tool_event("tool_system_shutdown", {
                "timestamp": datetime.now().isoformat()
            })
