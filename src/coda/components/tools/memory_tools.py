...
class StoreMemoryTool(BaseTool):
    ...
    async def _execute_impl(self, parameters: Dict[str, Any], context: Optional[Dict[str, Any]] = None) -> str:
        """Execute the tool."""
...
class SearchMemoryTool(BaseTool):
    ...
    async def _execute_impl(self, parameters: Dict[str, Any], context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """Execute the tool."""
    ...
    def _create_definition(self) -> ToolDefinition:
        ...
        return create_simple_tool_definition(
            ...
            parameters=[
                ...
                self.create_boolean_parameter(
                    "include_tags",
                    "Include breakdown by tags",
                    required=False,
                    default=True
                ),
                self.create_boolean_parameter(
                    "include_recent",
                    "Include recent memory activity",
                    required=False,
                    default=True
                )
            ],
            examples=[
                {"include_categories": True, "include_tags": True},
                {"include_recent": True}
            ],
...
class DeleteMemoryTool(BaseTool):
    ...
    def _create_definition(self) -> ToolDefinition:
        ...
        return create_simple_tool_definition(
            ...
            examples=[
                {"memory_id": "mem_123456", "confirm": True}
            ],
...
class UpdateMemoryTool(BaseTool):
    ...
    async def _execute_impl(self, parameters: Dict[str, Any], context: Optional[Dict[str, Any]] = None) -> str:
        ...
        if not any([content, tags, importance is not None]):
            raise ValueError("At least one field (content, tags, or importance) must be provided for update")
