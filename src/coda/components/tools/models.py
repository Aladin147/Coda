...
class ToolParameter(BaseModel):
    ...
    def validate_default_type(self):
        ...
        if param_type == ParameterType.STRING and not isinstance(self.default, str):
            raise ValueError(f"Default value must be string for string parameter")
        elif param_type == ParameterType.INTEGER and not isinstance(self.default, int):
            raise ValueError(f"Default value must be integer for integer parameter")
        elif param_type == ParameterType.FLOAT and not isinstance(self.default, (int, float)):
            raise ValueError(f"Default value must be number for float parameter")
        elif param_type == ParameterType.BOOLEAN and not isinstance(self.default, bool):
            raise ValueError(f"Default value must be boolean for boolean parameter")
        elif param_type == ParameterType.ARRAY and not isinstance(self.default, list):
            raise ValueError(f"Default value must be array for array parameter")
        elif param_type == ParameterType.OBJECT and not isinstance(self.default, dict):
