...
from .base_tool import ToolError, ToolExecutionError, ToolTimeoutError
...
class ToolExecutor(ToolExecutorInterface):
    ...
    def __init__(
        self, registry: ToolRegistryInterface, config: Optional[ToolExecutorConfig] = None
    ):
        """
        ...
        logger.info(
            f"ToolExecutor initialized with max {self.config.max_concurrent_executions} concurrent executions"
        )
    ...
    def get_execution_stats(self) -> ToolStats:
        ...
        successful_executions = sum(
            1 for exec_data in self._execution_history if exec_data.get("success", False)
        )
        failed_executions = total_executions - successful_executions

        # Calculate average execution time
        execution_times = [
            exec_data.get("execution_time_ms", 0) for exec_data in self._execution_history
        ]
        ...
        for attempt in range(execution.max_attempts):
            try:
                ...
                async with self._execution_semaphore:
                    ...
                    # Execute the tool
                    result = await tool.execute(execution.call.parameters, execution.call.context)
                ...
                last_error = str(e)
                logger.warning(
                    f"Tool {execution.call.tool_name} timed out (attempt {attempt + 1}/{execution.max_attempts})"
                )
                ...
                last_error = str(e)
                logger.warning(
                    f"Tool {execution.call.tool_name} failed: {e} (attempt {attempt + 1}/{execution.max_attempts})"
                )
    ...
    def _record_execution(self, execution: ToolExecution, result: ToolResult) -> None:
        ...
        if self.config.performance_monitoring:
            ...
            if len(self._performance_metrics[execution.call.tool_name]) > 100:
                self._performance_metrics[execution.call.tool_name] = self._performance_metrics[
