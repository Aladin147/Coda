...
from .interfaces import ToolRegistryInterface, ToolInterface
from .models import (
    ToolDefinition,
    ToolCall,
    ToolCategory,
...
class ToolRegistry(ToolRegistryInterface):
    ...
    def register_tool(self, tool: ToolInterface) -> bool:
        ...
        try:
            ...
            if len(self._tools) >= self.config.max_tools:
                logger.error(f"Cannot register tool {tool_name}: maximum tools limit reached ({self.config.max_tools})")
                return False
            ...
            if definition.is_dangerous and not self.config.allow_dangerous_tools:
                logger.error(f"Cannot register dangerous tool {tool_name}: dangerous tools not allowed")
                return False
            ...
            for alias in definition.aliases:
                if alias in self._aliases:
                    logger.warning(f"Alias {alias} already exists for tool {self._aliases[alias]}, overriding")
    ...
    def search_tools(self, query: str) -> List[ToolDefinition]:
        ...
        matches.sort(key=lambda d: (
            0 if query_lower == d.name.lower() else
            1 if query_lower in d.name.lower() else
            2 if query_lower in d.description.lower() else
            3
        ))
    ...
    def validate_tool_call(self, tool_call: ToolCall) -> bool:
        ...
        try:
            ...
            if not tool or not tool.is_available():
                logger.warning(f"Tool call validation failed: tool {tool_call.tool_name} not available")
                return False
        ...
        try:
            ...
            for param in definition.parameters:
                if param.name in parameter_names:
                    logger.error(f"Duplicate parameter name: {param.name} in tool {definition.name}")
                    return False
                parameter_names.add(param.name)
            
            # Check timeout
            if definition.timeout_seconds <= 0:
                logger.error(f"Invalid timeout: {definition.timeout_seconds} for tool {definition.name}")
                return False
    ...
    def export_registry(self) -> Dict[str, Any]:
        ...
        return {
            "tools": {name: definition.model_dump() for name, definition in self._definitions.items()},
