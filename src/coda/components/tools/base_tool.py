...
from .models import ParameterType, ToolCategory, ToolDefinition, ToolParameter
...
class BaseTool(ToolInterface):
    ...
    async def execute(
        ...
        if not self.is_available():
            raise ToolExecutionError(
                f"Tool {self._definition.name} is not available", self._definition.name
            )
        ...
        try:
            ...
            result = await asyncio.wait_for(
                self._execute_impl(validated_params, context), timeout=timeout
            )
    ...
    def create_boolean_parameter(
        ...
        examples: Optional[List[bool]] = None,
    ) -> ToolParameter:
    ...
    def validate_positive_number(self, value, parameter_name: str):
        ...
        if value <= 0:
            raise ToolValidationError(
                f"Parameter '{parameter_name}' must be positive",
                parameter_name,
                self._definition.name,
            )
        return value
    ...
    def get_context_value(
        self, context: Optional[Dict[str, Any]], key: str, default: Any = None
    ) -> Any:
...
def create_simple_tool_definition(
    name: str,
    description: str,
