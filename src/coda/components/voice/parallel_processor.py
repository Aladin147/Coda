...
class WorkerPool:
        ...
        try:
            if self.config.worker_type == "process":
                ...
                self.executor = ThreadPoolExecutor(
                    max_workers=self.config.max_workers,
                    thread_name_prefix="voice_worker"
                )
            ...
            logger.info(f"WorkerPool initialized with {self.config.max_workers} {self.config.worker_type} workers")
    ...
    async def submit_request(
        ...
        # Submit to executor
        future = loop.run_in_executor(self.executor, wrapped_processor)
