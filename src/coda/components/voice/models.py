...
class MoshiConfig(BaseModel):
    ...
    model_path: Optional[str] = Field(
        default="kyutai / moshiko - pytorch - bf16", description="Moshi model path"
    )
    ...
    max_conversation_length: int = Field(
        default=300, description="Max conversation length in seconds"
    )
    ...
    inner_monologue_enabled: bool = Field(
        default=True, description="Enable inner monologue text extraction"
    )
...
class VoiceConfig(BaseModel):
    ...
    # Core settings
    mode: VoiceProcessingMode = Field(
        default=VoiceProcessingMode.HYBRID, description="Voice processing mode"
    )
    conversation_mode: ConversationMode = Field(
        default=ConversationMode.FULL_DUPLEX, description="Conversation mode"
    )
    ...
    external_llm: ExternalLLMConfig = Field(
        default_factory=ExternalLLMConfig, description="External LLM configuration"
    )

    # Integration settings
    memory_integration_enabled: bool = Field(default=True, description="Enable memory integration")
    personality_integration_enabled: bool = Field(
        default=True, description="Enable personality integration"
    )
    ...
    # Fallback settings
    enable_traditional_pipeline: bool = Field(
        default=False, description="Enable traditional STT / TTS fallback"
    )
...
class VoiceMessage(BaseModel):
    ...
    memory_context: Optional[Dict[str, Any]] = Field(
        default=None, description="Memory context used"
    )
    personality_state: Optional[Dict[str, Any]] = Field(
        default=None, description="Personality state"
    )
...
class VoiceResponse(BaseModel):
    ...
    response_relevance: Optional[float] = Field(
        default=None, description="Response relevance score"
    )

    # Integration results
    memory_updates: Optional[List[Dict[str, Any]]] = Field(
        default=None, description="Memory updates made"
    )
    personality_changes: Optional[Dict[str, Any]] = Field(
        default=None, description="Personality changes"
    )
...
class VoiceStreamChunk(BaseModel):
    ...
    text_content: Optional[str] = Field(default=None, description="Text content for this chunk")
    ...
    processing_latency_ms: Optional[float] = Field(
        default=None, description="Chunk processing latency"
    )
...
class ConversationState(BaseModel):
    ...
    # Context
    memory_context: Optional[Dict[str, Any]] = Field(
        default=None, description="Current memory context"
    )
    personality_state: Optional[Dict[str, Any]] = Field(
        default=None, description="Current personality state"
    )
...
class VoiceAnalytics(BaseModel):
    ...
    memory_integration_usage: float = Field(
        default=0.0, description="Memory integration usage rate"
    )
    personality_integration_usage: float = Field(
        default=0.0, description="Personality integration usage rate"
    )
