...
class PerformanceMonitor:
    ...
    def get_average_metrics(self, window_minutes: int = 5) -> Optional[PerformanceMetrics]:
        ...
        return PerformanceMetrics(
            ...
            throughput_chunks_per_sec=sum(m.throughput_chunks_per_sec for m in recent_metrics)
            / len(recent_metrics),
            timestamp=datetime.now(),
        )
...
class AudioUtils:
    ...
    def calculate_audio_duration(
        audio_data: bytes, sample_rate: int, channels: int = 1, bit_depth: int = 16
    ) -> float:
    ...
    def calculate_audio_size(
        duration_seconds: float, sample_rate: int, channels: int = 1, bit_depth: int = 16
    ) -> int:
...
class StreamingUtils:
    ...
    async def chunk_audio_stream(
        audio_stream: AsyncGenerator[bytes, None], chunk_size: int = 1024
    ...
    async def buffer_audio_stream(
        ...
        buffer_size = AudioUtils.calculate_audio_size(buffer_duration_ms / 1000.0, sample_rate)
...
# Removed duplicate LatencyContext class - using the one defined above
