...
class VoiceConversationSync:
    ...
    async def sync_voice_message(
        ...
        try:
            ...
            await self.conversation_manager.add_message(
                conversation_id=voice_message.conversation_id, message=conversation_message
            )
    ...
    async def sync_voice_response(
        ...
        try:
            ...
            conversation_message = await self._convert_voice_response_to_conversation_message(
                voice_response
            )

            # Add message to conversation
            await self.conversation_manager.add_message(
                conversation_id=voice_response.conversation_id, message=conversation_message
            )
            ...
            # Emit sync event
            await self._emit_sync_event(
                SyncEventType.VOICE_RESPONSE_ADDED,
                {
    ...
    async def sync_conversation_state(self, conversation_state: ConversationState) -> bool:
        """
        ...
        try:
            ...
            await self.conversation_manager.update_conversation_metadata(
                conversation_id=conversation_state.conversation_id, metadata=metadata
            )
            ...
            # Emit sync event
            await self._emit_sync_event(
                SyncEventType.CONVERSATION_STATE_UPDATED,
                {
        ...
        try:
            ...
            if await self._detect_sync_conflict(conversation_id, update_data):
                ...
                if not conflict_resolved:
                    logger.warning(
                        f"Failed to resolve sync conflict for conversation {conversation_id}"
                    )
                    return False
    ...
    async def _convert_voice_response_to_conversation_message(
    ...
    async def _detect_sync_conflict(
        ...
        if last_sync and isinstance(update_timestamp, datetime):
            ...
            if last_sync > update_timestamp:
                ...
                await self._emit_sync_event(
                    SyncEventType.SYNC_CONFLICT_DETECTED,
                    {
        ...
        try:
            ...
            if strategy == ConflictResolutionStrategy.VOICE_PRIORITY:
                ...
                if update_timestamp > last_sync:
                    logger.debug(
                        f"Resolving conflict with timestamp priority (update wins) for {conversation_id}"
                    )
                    return True
                else:
                    logger.debug(
                        f"Resolving conflict with timestamp priority (voice wins) for {conversation_id}"
                    )
                    return True
    ...
    async def _merge_conversation_updates(
        self, conversation_id: str, update_data: Dict[str, Any]
    ) -> bool:
    ...
    async def _process_sync_operation(self, operation: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if op_type == "voice_message":
                await self.sync_voice_message(
                    operation["voice_message"], operation.get("conversation_state")
                )
            elif op_type == "voice_response":
                await self.sync_voice_response(
                    operation["voice_response"], operation.get("conversation_state")
                )
...
class VoiceConversationSyncManager:
    ...
    async def sync_voice_interaction(
        ...
        try:
            ...
            # Sync voice response
            response_synced = await self.sync.sync_voice_response(
                voice_response, conversation_state
            )
