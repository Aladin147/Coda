...
from .models import VoiceMessage, VoiceProcessingMode, VoiceResponse
...
class LatencyOptimizationConfig:
    ...
    enabled_techniques: List[OptimizationTechnique] = field(
...
class LatencyProfiler:
    ...
    def record_component_latency(self, component: str, latency_ms: float) -> None:
        ...
        if len(self.component_stats[component]) > self.config.measurement_window_size:
            self.component_stats[component] = self.component_stats[component][
                -self.config.measurement_window_size :
            ]
    ...
    def get_latency_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            "overall": {
                ...
                "std_dev_ms": (
                    statistics.stdev(total_latencies) if len(total_latencies) > 1 else 0.0
                ),
                "p95_ms": (
                    sorted(total_latencies)[int(len(total_latencies) * 0.95)]
                    if total_latencies
                    else 0.0
                ),
                "p99_ms": (
                    sorted(total_latencies)[int(len(total_latencies) * 0.99)]
                    if total_latencies
                    else 0.0
                ),
            },
        ...
        for component, latencies in self.component_stats.items():
            if latencies and total_latencies:
                ...
                contribution_percent = (
                    (mean_component / mean_total) * 100 if mean_total > 0 else 0.0
                )
...
class LatencyOptimizer:
    ...
    def __init__(
        ...
        self.current_optimizations: Dict[OptimizationTechnique, bool] = {
            technique: technique in config.enabled_techniques for technique in OptimizationTechnique
        }

        logger.info(
            f"LatencyOptimizer initialized with target: {config.target_latency} ({config.target_latency_ms}ms)"
        )
    ...
    async def _apply_postprocessing_optimizations(self, response: VoiceResponse) -> VoiceResponse:
        ...
        if (
            self.current_optimizations[OptimizationTechnique.EARLY_TERMINATION]
            and response.response_relevance
            and response.response_relevance >= self.config.early_termination_confidence
        ):
    ...
    async def _process_with_pipeline_optimization(
        self, voice_message: VoiceMessage, processing_callback: Callable
    ) -> VoiceResponse:
        ...
        if self.parallel_processor:
            ...
            return await self.parallel_processor.get_response(
                request_id, timeout=self.config.target_latency_ms / 1000.0
            )
        else:
            return await processing_callback(voice_message)
    ...
    def get_optimization_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_optimizations": [
                technique.value
                for technique, enabled in self.current_optimizations.items()
                if enabled
            ],
