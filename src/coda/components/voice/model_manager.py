...
class VoiceModelManager:
    ...
    def __init__(self, moshi_vram_usage: float = 15.0):
        ...
        logger.info(
            f"VoiceModelManager initialized - Total VRAM: {self.total_vram_gb:.1f}GB, "
    ...
    def get_optimal_model(self, context: Optional[Dict[str, Any]] = None) -> str:
        ...
        """
        use_case = (
            context.get("use_case", "general_conversation") if context else "general_conversation"
        )
    ...
    async def load_model(self, model_name: str) -> VoiceLLMProcessor:
        ...
        if required_vram > available_vram:
            ...
            if required_vram > available_vram:
                raise RuntimeError(
                    f"Insufficient VRAM for {model_name}: need {required_vram:.1f}GB, "
    ...
    async def _free_vram(self, required_gb: float) -> None:
        ...
        for name, processor in self.loaded_models.items():
            ...
            models_by_priority.append(
                (priority_order[model_info.priority], name, model_info.estimated_vram_gb)
            )
