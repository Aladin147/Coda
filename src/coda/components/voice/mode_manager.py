...
class ProcessingModeManager:
        ...
        try:
            ...
            modes_to_init = [
                ...
                (
                    ...
                    LLMOnlyMode(self.context_config, self.llm_config, self.optimization_config),
                ),
                (
                ...
                (
                    ProcessingModeType.HYBRID_QUALITY,
                    HybridQualityMode(
                        hybrid_config,
                        self.moshi_config,
                        self.context_config,
    ...
    async def _auto_select_mode(
        ...
        if word_count > 50 or any(
            word in text_lower for word in ["complex", "detailed", "comprehensive"]
        ):
            complexity = "high"
    ...
    def _track_mode_performance(
        self, mode_type: ProcessingModeType, processing_time_ms: float, response: VoiceResponse
    ) -> None:
    ...
    def get_available_modes(self) -> List[ProcessingModeConfig]:
        ...
        return [mode_instance.get_mode_info() for mode_instance in self.available_modes.values()]
    ...
    def get_mode_performance_stats(self) -> Dict[str, Any]:
        ...
        for mode_type, performance_data in self.mode_performance.items():
            ...
            avg_processing_time = sum(p["processing_time_ms"] for p in recent_data) / len(
                recent_data
            )
