...
from .models import MoshiConfig, VoiceConfig
...
class InnerMonologueProcessor:
        ...
        with track_latency(self.latency_tracker) as tracker:
            try:
                ...
                if extracted_text and extracted_text.confidence >= self.confidence_threshold:
                    ...
                    logger.debug(
                        f"Extracted text: '{extracted_text.text}' (confidence: {extracted_text.confidence:.3f})"
                    )
    ...
    async def get_continuous_text(self, time_window_ms: Optional[float] = None) -> str:
        ...
        recent_items = [
            item for item in self.text_buffer if item.timestamp.timestamp() >= cutoff_time
        ]
    ...
    async def _extract_audio_features(self, audio_tensor: torch.Tensor) -> torch.Tensor:
        ...
        try:
            # Ensure audio tensor is on correct device and has proper shape
            ...
            # Use Moshi client for feature extraction if available
            if hasattr(self, 'moshi_client') and self.moshi_client and self.moshi_client.is_initialized:
                try:
                    # Convert tensor to bytes for Moshi processing
                    audio_bytes = self.moshi_client._tensor_to_bytes(audio_tensor.squeeze(0))

                    # Extract features using Moshi's compression model
                    if (hasattr(self.moshi_client, 'compression_model') and
                        self.moshi_client.compression_model is not None):

                        with torch.no_grad():
                            # Encode audio to compressed representation (features)
                            compressed_features = self.moshi_client.compression_model.encode(audio_tensor)

                            # Flatten features for consistent output shape
                            if compressed_features.dim() > 2:
                                compressed_features = compressed_features.view(batch_size, -1)

                            logger.debug(f"Extracted audio features via Moshi compression, shape: {compressed_features.shape}")
                            return compressed_features

                except Exception as e:
                    logger.warning(f"Moshi feature extraction failed: {e}")
                    # Fall through to fallback

            # Fallback: Extract basic spectral features
            feature_dim = 512
            features = self._extract_spectral_features(audio_tensor, feature_dim)
            logger.debug(f"Extracted fallback spectral features, shape: {features.shape}")
            return features
    ...
    def _extract_spectral_features(self, audio_tensor: torch.Tensor, feature_dim: int) -> torch.Tensor:
        """Extract spectral features as fallback when Moshi is not available."""
        try:
            # Ensure audio is 2D (batch, samples)
            if audio_tensor.dim() == 3:
                audio_tensor = audio_tensor.squeeze(1)  # Remove channel dim
            elif audio_tensor.dim() == 1:
                audio_tensor = audio_tensor.unsqueeze(0)  # Add batch dim

            batch_size, seq_len = audio_tensor.shape

            # Compute basic spectral features using FFT
            with torch.no_grad():
                # Apply window to reduce spectral leakage
                window_size = min(seq_len, 1024)
                window = torch.hann_window(window_size).to(audio_tensor.device)
                ...
                if seq_len >= 1024:
                    ...
                    spectral_features = torch.mean(magnitude, dim=-1)
                else:
                    # For short audio, use simple FFT
                    fft = torch.fft.fft(audio_tensor, n=1024, dim=-1)
                    spectral_features = torch.abs(fft[:, :512])  # Take first half (positive frequencies)

                # Reduce to target feature dimension
                if spectral_features.shape[-1] > feature_dim:
                    # Downsample features
                    step = spectral_features.shape[-1] // feature_dim
                    features = spectral_features[:, ::step][:, :feature_dim]
                elif spectral_features.shape[-1] < feature_dim:
                    # Pad features
                    padding = feature_dim - spectral_features.shape[-1]
                    features = torch.nn.functional.pad(spectral_features, (0, padding))
                else:
            ...
            # Return zero features as final fallback
            return torch.zeros(audio_tensor.shape[0], feature_dim).to(audio_tensor.device)
    ...
    def _update_statistics(self, extracted_text: ExtractedText, audio_duration_ms: float) -> None:
        ...
        if self.state.total_extractions == 1:
            ...
            self.state.average_confidence = (
                self.state.average_confidence * (self.state.total_extractions - 1)
        ...
        if self.state.total_extractions == 1:
            ...
            self.state.average_latency_ms = (
                self.state.average_latency_ms * (self.state.total_extractions - 1)
                + extracted_text.extraction_latency_ms
            ) / self.state.total_extractions
...
class InnerMonologueManager:
    ...
    async def create_processor(
        self, processor_id: str, config: Optional[MoshiConfig] = None
    ...
    async def initialize_processor(
        self, processor_id: str, moshi_model: Any, tokenizer: Any = None
    ) -> None:
