...
class MoshiClient:
    ...
    def _extract_mock_inner_monologue(self, audio_array: np.ndarray) -> str:
        ...
        if audio_energy < 1000:
            ...
            inner_monologue = (
                "That was a very short utterance, probably a greeting or simple question."
            )
        elif audio_length > 10.0:
            inner_monologue = (
                "The user spoke for a long time, this seems like a complex request or explanation."
            )
        else:
            inner_monologue = (
                "The user is asking me something, I should provide a helpful response."
            )
    ...
    async def stream_audio(
        self, audio_stream: asyncio.Queue, conversation_id: Optional[str] = None
    ) -> asyncio.Queue:
        """
    ...
    def get_conversation_history(
        self, conversation_id: Optional[str] = None, limit: int = 10
    ) -> List[Dict[str, Any]]:
    ...
    async def start_websocket_server(self) -> None:
        ...
        logger.info(
            f"WebSocket server would start on {self.config.websocket_host}:{self.config.websocket_port}"
        )
