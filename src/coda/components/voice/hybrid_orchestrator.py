...
from .models import VoiceMessage, VoiceResponse, VoiceProcessingMode, ConversationState
from .moshi_client import MoshiClient, MoshiConfig
from .context_integration import VoiceContextManager, ContextConfig
...
class HybridOrchestrator:
    ...
    def __init__(
        ...
        logger.info(f"HybridOrchestrator initialized with {hybrid_config.default_strategy} strategy")
    ...
    async def _execute_strategy(
        ...
        if strategy == ProcessingStrategy.MOSHI_ONLY:
            ...
            return await self._process_hybrid_parallel(voice_message, conversation_state, request_id)
    ...
    async def _process_hybrid_sequential(
        ...
        try:
            ...
            if (moshi_result.quality_score >= self.config.min_response_quality and
                moshi_result.relevance_score >= self.config.min_relevance_score):
                ...
                if remaining_time > 500:  # At least 500ms for LLM enhancement
                    try:
                        ...
                        return ProcessingResult(
                            ...
                            relevance_score=max(moshi_result.relevance_score, llm_result.relevance_score),
                            confidence=0.85,
                            metadata={
    ...
    def _combine_responses(
        ...
        combined_response = VoiceResponse(
            ...
            response_relevance=max(
                moshi_response.response_relevance or 0.5,
                llm_response.response_relevance or 0.5
            )
        )
    ...
    def _update_performance_stats(
        ...
        if len(self.strategy_performance[strategy]) > self.config.performance_history_size:
            self.strategy_performance[strategy] = self.strategy_performance[strategy][-self.config.performance_history_size:]
    ...
    async def _update_adaptive_learning(
        ...
        # If performance is poor, consider switching
            ...
            if best_strategy and best_avg > performance_score:
                logger.info(f"Adaptive learning: switching from {self.current_strategy} to {best_strategy}")
