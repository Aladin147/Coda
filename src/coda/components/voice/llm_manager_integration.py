...
class VoiceLLMManagerIntegration:
    ...
    def __init__(self, llm_manager: LLMManagerInterface, config: VoiceLLMManagerConfig):
    ...
    async def _get_llm_conversation_id(self, voice_conversation_id: str) -> str:
        ...
        logger.debug(
            f"Created LLM conversation {llm_conversation_id} for voice conversation {voice_conversation_id}"
        )

        return llm_conversation_id

    async def _prepare_voice_prompt(
        self, voice_message: VoiceMessage, enhanced_context: Optional[Dict[str, Any]] = None
    ) -> str:
    ...
    async def _convert_llm_to_voice_response(
        self, llm_response: LLMResponse, voice_message: VoiceMessage, start_time: float
    ) -> VoiceResponse:
        """Convert LLM response to voice response."""
    ...
    async def _create_fallback_response(
        self, voice_message: VoiceMessage, error_message: str
    ) -> VoiceResponse:
    ...
    def _generate_cache_key(
        self, voice_message: VoiceMessage, enhanced_context: Optional[Dict[str, Any]]
    ) -> str:
    ...
    def _update_average_response_time(self, new_time_ms: float) -> None:
        ...
        if request_count == 1:
            ...
            self.stats["average_response_time_ms"] = (
                current_avg * (request_count - 1) + new_time_ms
            ) / request_count

    async def get_conversation_history(
        self, voice_conversation_id: str, limit: int = 10
    ) -> List[LLMMessage]:
...
class VoiceLLMManager:
    ...
    def __init__(
        self, llm_manager: LLMManagerInterface, config: Optional[VoiceLLMManagerConfig] = None
    ):
