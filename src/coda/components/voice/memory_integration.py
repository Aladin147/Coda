...
from ..memory.models import (
    ConversationTurn, ConversationContext, MemoryQuery, MemoryResult,
    ConversationRole, MemoryType
)
...
class VoiceMemoryIntegration:
    ...
    async def _get_memory_context(self, voice_message: VoiceMessage) -> Dict[str, Any]:
        ...
        try:
            ...
            for result in memory_results:
                if result.final_score >= self.config.memory_relevance_threshold:
                    memory_info = {
                        ...
                        "timestamp": result.memory.created_at.isoformat() if result.memory.created_at else None,
            ...
            memory_context = {
                ...
                "average_relevance": total_relevance / len(relevant_memories) if relevant_memories else 0.0,
    ...
    async def _get_enhanced_conversation_context(self, voice_message: VoiceMessage) -> Dict[str, Any]:
    ...
    async def _check_consolidation(self) -> None:
        ...
        if (self.config.enable_auto_consolidation and 
            self.message_count >= self.config.consolidation_interval):
