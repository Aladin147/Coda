...
class VRAMMonitor:
    ...
    def __init__(self, device_id: int = 0):
        ...
        if torch.cuda.is_available():
            self.total_vram_mb = torch.cuda.get_device_properties(device_id).total_memory / (
                1024 * 1024
            )
    ...
    def get_detailed_info(self) -> Dict[str, Any]:
        ...
        info = {
            ...
            "device_name": (
                torch.cuda.get_device_name(self.device_id) if torch.cuda.is_available() else "CPU"
            ),
    ...
    def get_history(self, minutes: int = 10) -> List[VRAMAllocation]:
        ...
        return [
            allocation
            for allocation in self.metrics_history
...
class DynamicVRAMManager:
    ...
    def __init__(self, config: VoiceConfig):
        ...
        logger.info(
            f"VRAM Manager initialized: {self.total_vram_mb:.0f}MB total, "
            f"{self.available_vram_mb:.0f}MB available"
        )
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            if current.total_mb < self.total_vram_mb * 0.9:
                logger.warning(
                    f"Detected VRAM ({current.total_mb:.0f}MB) less than configured ({self.total_vram_mb:.0f}MB)"
                )
    ...
    def register_component(
        self, component_id: str, max_mb: float, priority: int = 5, can_resize: bool = True
    ) -> bool:
        ...
        with self.allocation_lock:
            ...
            if total_allocated + max_mb > self.available_vram_mb:
                logger.error(
                    f"Not enough VRAM for {component_id}: need {max_mb}MB, "
                    f"available {self.available_vram_mb - total_allocated}MB"
                )
                return False
            ...
            self._record_event(event_type="register", component_id=component_id, size_mb=max_mb)
    ...
    def allocate(self, component_id: str, size_mb: float) -> bool:
        ...
        with self.allocation_lock:
            ...
            if allocation.allocated_mb + size_mb > allocation.max_mb:
                logger.error(
                    f"Allocation exceeds maximum for {component_id}: {allocation.allocated_mb + size_mb}MB > {allocation.max_mb}MB"
                )
                return False
            ...
            try:
                ...
                self._record_event(
                    event_type="allocate", component_id=component_id, size_mb=size_mb
                )

                logger.debug(
                    f"Allocated {size_mb}MB for {component_id}, total: {allocation.allocated_mb}MB"
                )
                return True
    ...
    def deallocate(self, component_id: str, size_mb: Optional[float] = None) -> bool:
        ...
        with self.allocation_lock:
            ...
            try:
                ...
                self._record_event(
                    event_type="deallocate", component_id=component_id, size_mb=size_mb
                )

                logger.debug(
                    f"Deallocated {size_mb}MB for {component_id}, remaining: {allocation.allocated_mb}MB"
                )
                return True
    ...
    def unregister_component(self, component_id: str) -> bool:
        ...
        with self.allocation_lock:
            ...
            self._record_event(event_type="unregister", component_id=component_id, size_mb=0.0)
    ...
    def optimize_memory(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Memory optimization completed: freed {optimization_results['freed_mb']:.1f}MB"
            )
    ...
    def _handle_memory_pressure(self, pressure: MemoryPressure) -> float:
        ...
        if pressure in [MemoryPressure.HIGH, MemoryPressure.CRITICAL]:
            with self.allocation_lock:
                ...
                components = sorted(
                    self.allocations.items(), key=lambda x: (x[1].priority, x[1].last_used)
                )
                ...
                for comp_id, allocation in components:
                    if allocation.can_resize and allocation.allocated_mb > 0:
                        ...
                        if self.deallocate(comp_id, reduction):
                            freed_mb += reduction
                            logger.info(
                                f"Reduced {comp_id} allocation by {reduction:.1f}MB due to memory pressure"
                            )
