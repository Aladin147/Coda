...
class ConversationStateManager:
        ...
        with self.state_lock:
            ...
            if metrics.total_exchanges == 1:
                ...
                metrics.average_response_latency_ms = (
                    metrics.average_response_latency_ms * (metrics.total_exchanges - 1)
            ...
            if confidence_score is not None:
                if metrics.total_exchanges == 1:
                    ...
                    metrics.average_confidence_score = (
                        metrics.average_confidence_score * (metrics.total_exchanges - 1)
                        + confidence_score
                    ) / metrics.total_exchanges
    ...
    def add_inner_monologue_text(self, conversation_id: str, extracted_text: ExtractedText) -> None:
        ...
        with self.state_lock:
            ...
            self._record_event(
                ...
                data={"text": extracted_text.text, "confidence": extracted_text.confidence},
                phase=ConversationPhase.PROCESSING,
            )
    ...
    def _record_event(
        self, conversation_id: str, event_type: str, data: Dict[str, Any], phase: ConversationPhase
    ) -> None:
    ...
    def _calculate_quality_score(self, metrics: ConversationMetrics) -> float:
        ...
        if metrics.average_response_latency_ms > 0:
            latency_score = max(
                0, 1.0 - (metrics.average_response_latency_ms / 1000.0)
            )  # 1s = 0 score
