...
from ..personality.models import PersonalityTrait, PersonalityParameters
...
class ContextInjector:
    ...
    async def build_context(
        ...
        if self.config.parallel_context_loading:
            ...
            result_index = 0
            if self.config.enable_memory_injection:
                context["memory"] = results[result_index] if not isinstance(results[result_index], Exception) else {}
                result_index += 1
            
            if self.config.enable_personality_injection:
                context["personality"] = results[result_index] if not isinstance(results[result_index], Exception) else {}
                result_index += 1
            
            context["conversation"] = results[result_index] if not isinstance(results[result_index], Exception) else {}
            ...
            context["conversation"] = await self._load_conversation_context(conversation_id, conversation_state)
            context["current_message"] = self._extract_message_context(voice_message)
    ...
    async def _load_memory_context(self, voice_message: VoiceMessage) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(f"Loaded memory context with {len(memory_context['recent_topics'])} topics")
        ...
        try:
            ...
            if conversation_state:
                conversation_context.update({
                    "message_count": len(conversation_state.messages) if hasattr(conversation_state, 'messages') else 0,
                    "is_active": conversation_state.is_active,
                    "start_time": conversation_state.start_time.isoformat() if hasattr(conversation_state, 'start_time') else None
                })
    ...
    def _extract_message_context(self, voice_message: VoiceMessage) -> Dict[str, Any]:
        ...
        try:
            message_context = {
                ...
                "audio_length_seconds": len(voice_message.audio_data) / (24000 * 2) if voice_message.audio_data else 0,
    ...
    def _estimate_complexity(self, text: str) -> str:
        ...
        complex_keywords = [
            "implement", "algorithm", "architecture", "optimization", "performance",
            "integration", "system", "design", "analysis", "research", "compare",
            "explain", "detailed", "comprehensive", "advanced"
        ]
...
class VoiceContextManager:
    ...
    def __init__(
        ...
        self.model_manager = VoiceModelManager(moshi_vram_usage) if context_config.enable_dynamic_models else None
    ...
    async def process_with_context(
        ...
        try:
            ...
            if self.current_processor:
                return await self.current_processor.process_voice_message(voice_message, conversation_state)
            raise
