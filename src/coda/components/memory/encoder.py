...
class MemoryEncoder(MemoryEncoderInterface):
    ...
    def __init__(self, config: Optional[MemoryEncoderConfig] = None):
        ...
        self._importance_pattern = re.compile(
            r'\b(' + '|'.join(self.config.importance_keywords) + r')\b',
            re.IGNORECASE
        )
    ...
    def encode_conversation(self, turns: List[ConversationTurn], 
                           include_system: bool = False) -> List[MemoryChunk]:
        """
    ...
    def encode_fact(self, fact: str, source: str = "user", 
                   metadata: Optional[Dict[str, Any]] = None) -> MemoryChunk:
        """
    ...
    def calculate_importance(self, content: str) -> float:
        ...
        final_score = base_score + keyword_boost + length_factor + question_boost + exclamation_boost
    ...
    def _group_consecutive_turns(self, turns: List[ConversationTurn]) -> List[List[ConversationTurn]]:
        """Group consecutive turns by the same speaker."""
        if not turns:
            return []
    ...
    def _create_chunks_from_group(self, group: List[ConversationTurn]) -> List[MemoryChunk]:
        ...
        topics = self.extract_topics(combined_content) if self.config.topic_extraction_enabled else []
    ...
    def _is_common_word(self, word: str) -> bool:
        ...
        common_words = {
            ...
            'should', 'now', 'hello', 'hi', 'hey', 'okay', 'ok', 'yes', 'no',
            'please', 'thank', 'thanks', 'sorry', 'excuse', 'well', 'actually',
            'really', 'maybe', 'perhaps', 'probably', 'definitely', 'certainly'
        }
