...
class ShortTermMemory(ShortTermMemoryInterface):
    ...
    def get_context(self, max_tokens: int = 800) -> ConversationContext:
            ...
            if system_turns:
                ...
                if system_tokens <= max_tokens:
                    messages.append(ContextMessage(
                        role=system_turn.role,
                        content=system_turn.content
                    ))
        ...
        for turn in reversed(non_system_turns):
            ...
            # Add turn to context (insert at beginning to maintain chronological order)
            messages.insert(-len([m for m in messages if m.role != ConversationRole.SYSTEM]), 
                          ContextMessage(role=turn.role, content=turn.content))
