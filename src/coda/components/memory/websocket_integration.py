...
class WebSocketMemoryManager(MemoryManager, WebSocketMemoryInterface):
    ...
    async def store_fact(self, fact: str, source: str = "user",
        ...
        await self.broadcast_memory_event("memory_store", {
            "memory_id": memory_id,
    ...
    async def search_memories(self, query: str, limit: int = 5,
        ...
        await self.broadcast_memory_event("memory_retrieve", {
            "query": query,
    ...
    async def consolidate_short_term(self) -> int:
        ...
        await self.broadcast_memory_event("memory_consolidate", {
    ...
    async def cleanup_old_memories(self, max_age_days: int = 365) -> int:
        ...
        await self.broadcast_memory_event("memory_cleanup", {
    ...
    async def delete_memory(self, memory_id: str) -> bool:
        ...
        if success and memory_info:
            ...
            await self.broadcast_memory_event("memory_delete", {
                "memory_id": memory_id,
                "content_preview": memory_info["content"][:100] + "..." if len(memory_info["content"]) > 100 else memory_info["content"],
    ...
    async def update_memory_importance(self, memory_id: str, importance: float) -> bool:
        ...
        if success:
            ...
            await self.broadcast_memory_event("memory_update", {
                "memory_id": memory_id,
                "field": "importance",
    ...
    async def broadcast_memory_stats(self) -> None:
        ...
        await self.broadcast_memory_event("memory_stats", {
    ...
    async def search_and_broadcast(self, query: str, limit: int = 5) -> List[MemoryResult]:
        ...
        for result in results:
            detailed_results.append({
                ...
                "content_preview": result.memory.content[:200] + "..." if len(result.memory.content) > 200 else result.memory.content,
        ...
        await self.broadcast_memory_event("memory_search_results", {
            "query": query,
